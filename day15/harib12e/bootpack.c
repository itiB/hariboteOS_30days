...
void task_b_main(struct SHEET *sht_back) {
    ...
    char s[11];
    int i, count, fifobuf[128];

    fifo32_init(&fifo, 128, fifobuf);
    timer_ts = timer_alloc();
    timer_init(timer_ts, &fifo, 2);
    timer_settime(timer_ts, 2); // 0.02秒でtask switch
...
void HariMain(void)
{
    ...
    // task_Bに飛んだ気にデータを入れておくためのスタック，
    // スタックには下から詰めるため一番下の番地を求めておく
    task_b_esp = memman_alloc_4k(memman, 64 * 1024) + 64 * 1024 - 8;
    // マルチタスク先にsht_backの情報を伝える
    //  引数はesp + 4に入る
    *((int *) (task_b_esp + 4)) = (int) sht_back;
    ...
    // マルチタスクで飛んだときのための設定
    tss_b.eip    = (int) &task_b_main;  // タスクが切り替わったらどこから実行してほしいか
    tss_b.eflags = 0x00000202;  // IF = 1 io_sti()のあとはこうなるらしい
    tss_b.eax    = 0;
    tss_b.ecx    = 0;
    tss_b.edx    = 0;
    tss_b.ebx    = 0;
    ...
    tss_b.esi    = 0;
    tss_b.edi    = 0;
    tss_b.es     = 1 * 8;   // 基本的にはbootpack.cのセグメントを使う
    tss_b.cs     = 2 * 8;
    tss_b.ss     = 1 * 8;
    tss_b.ds     = 1 * 8;
    tss_b.fs     = 1 * 8;
