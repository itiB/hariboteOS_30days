...
void close_constask(struct TASK *task) {
    struct MEMMAN *memman = (struct MEMMAN *) MEMMAN_ADDR;
    task_sleep(task); // タスクを実行順番待ちから切り離してスタックとFIFOを安全に解放
...
void HariMain(void) {
    ...
    *((int *) 0x0fe4) = (int) shtctl;
    ...
    if (finfo != 0) {
        ...
        for (i = 0; i < 16 * 256; i++) {
            // フォントがなかったので半角部分をコピーする
            nihongo[i] = hankaku[i];
        }
        for (i = 16 * 256; i < 16 * 256 + 32 * 94 * 47; i++) {
            // フォントが無かったので残りを0xffで埋め尽くす
            nihongo[i] = 0xff;
        }
    }
    ...
    *((int *) 0x0fec) = (int) &fifo; // タスクaの番地をCons消すときのために記録
            ...
            if (256 <= i && i <= 511) {   // キーボードデータ
                ...
                if (i == 256 + 0x3b && key_shift != 0 && key_win != 0) {
                    ...
                    if (task != 0 && task->tss.ss0 != 0) {
                        ...
                        io_sti();
                        task_run(task, -1, 0); // 終了処理を確実にするために寝ていたら起こす
                    }
                }
                if (i == 256 + 0x3c && key_shift != 0 && key_win != 0) {
                    // shift + f2
                                    ...
                                    if (sht->buf[y * sht->bxsize + x] != sht->col_inv) {
                                        ...
                                        if (sht->bxsize - 21 <= x && x < sht->bxsize - 5 && 5 <= y && y < 19) {
                                            ...
                                            if ((sht->flags & 0x10) != 0) {
                                                ...
                                                keywin_on(key_win);
                                                io_cli();
                                                // コンソールにfifoで4を通知する(本人にタイマやfatを解放させる)
                ...
                // コンソール終了処理
                close_console(shtctl->sheets0 + (i - 768)); // i-768でどの下敷きか計算
